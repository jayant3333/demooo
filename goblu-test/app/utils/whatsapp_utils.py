...
def generate_message(message_body,phone_number):
    if len(template_message_sent) == 0 :
...
def process_whatsapp_message(body):
    ...
    if(message_type == "text"):
        ...
        button_text = body['entry'][0]['changes'][0]['value']['messages'][0]['button']['text']
        message_from = body['entry'][0]['changes'][0]['value']['messages'][0]['from']
        response = generate_message(button_text,message_from)
